...
You can get an enumeration of MIDI devices on your machine by simply 
importing that library in a dclang interactive shell. For example:
...
PM_IN is not set in your environment!
MIDI input functionality will not work.
If you need this, please exit from dclang, set that value, and try again!
PM_OUT is not set in your environment!
MIDI output functionality will not work.
If you need this, please exit from dclang, set that value, and try again!
```

So, notice the message that we haven't selected a device. Now that we 
know what they are, let's say I want to use `ALSA, VirMIDI 1-0 
(output)`, a virtual port, to send data to. Simple, that's `PM_OUT=2`, 
so let's export that.

```
aaron@aaron-XPS-13-9350:~/programs/dclang/examples/midi$ export PM_OUT=2
```
...
defined in music_keyval.dc) to communicate variables to a live MIDI 
process. We can control these variables one of two ways: either directly 
change them via a shell script (see mset.sh), or -- start a little 
`dclang` web-server that serves a page with GUI widgets in your browser, 
and have AJAX messages update the same variables, which will be picked 
up by the separate `dclang` process that is running a MIDI script.

Here's how to start the server (you only type what's after the end of 
the shell prompt, which ends with `$`), where all this stuff will live 
will depend on how and where you've installed `dclang`:

```
aaron@aaron-XPS-13-9350:~/programs/dclang$ cd examples
aaron@aaron-XPS-13-9350:~/programs/dclang/examples$ dclang keyval_control.dc
running acceptloop; serving on port 7651 
```

The message tells us the interface is being served on port `7651`, so 
open you browser to `localhost:7651` in the address bar, and enjoy the 
widgets you'll see. Each will update a parameter behind-the-scenes. In a 
separate shell, you can start one of the example midi scripts that live 
in `examples/midi`. Note that this set of widgets covers most of the 
variables you'll encounter over the scripts, so not every parameter is 
used by any one script.
