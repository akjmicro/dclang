...
static int resize(size_t nel, struct hsearch_data *htab)
{
	...
	htab->__tab->entries = dclang_malloc(newsize*(sizeof *htab->__tab->entries));
...
static int __hcreate_r(size_t nel, struct hsearch_data *htab)
{
	...
	htab->__tab = dclang_malloc(sizeof *htab->__tab);
	if (!htab->__tab)
		return 0;
	r = resize(nel, htab);
	if (r == 0) {
		dclang_free(htab->__tab);
		htab->__tab = 0;
	}
	return r;
}
...
{
	if (htab->__tab) dclang_free(htab->__tab->entries);
	dclang_free(htab->__tab);
	htab->__tab = 0;
}
